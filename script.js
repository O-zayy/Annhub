...
function syncFullscreenPlayerMode() {
    ...
    // Keep custom cursor visible in fullscreen by re-parenting it
...
document.addEventListener('DOMContentLoaded', () => {
    const desktopDockContainer = document.getElementById('dock-container');
    const mobileDockContainer = document.getElementById('dock-container-mobile');
    if (!desktopDockContainer && !mobileDockContainer) return;
    ...
    if (mobileDockContainer) {
        new Dock('#dock-container-mobile', dockItems, {
            baseItemSize: 34,
            magnification: 48,
            spread: 80
        });
